module Pippi::Joruri::Importer::Facility
  class YochienShiritsuyochien < Pippi::Joruri::Importer::Base
    ...
    def import_facility_yochien_shiritsuyochien_docs
      csv = CSV.open("import_facility_yochien_shiritsuyochien_#{Time.zone.now.to_i}.csv",'w')
      path = ::File.join(csv_path, "facility/facilities.csv")
      facility_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      path = ::File.join(csv_path, "facility/yochien_shiritsuyochien.csv")
      yochien_shiritsuyochien_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      facility_csv.each_with_index do |row, idx|
        next unless row['category_id'] == '52' || row['category_id'] == '70'
        original_id = row["id"]
        ...
        node = yochien_nodes['私立幼稚園一覧']
        # category = yochien_nodes[category]
        category = yochien_nodes['私立幼稚園一覧']
        group = groups[group]
        user = groups[user]
        ...
        rel_Joruri = Pippi::Joruri::Relation::Facility::YochienShiritsuyochien.where(joruri_id: original_id).first
        if rel_Joruri
          item = rel_Joruri.owner_item
          next
        else
          item = Article::Page.new
          rel_Joruri = Pippi::Joruri::Relation::Facility::YochienShiritsuyochien.new
        end
        ...
        yochien_shiritsuyochien_row = yochien_shiritsuyochien_csv.find do |yochien_shiritsuyochien_row|
          next true if yochien_shiritsuyochien_row['施設名称'] == title
          yochien_shiritsuyochien_row['所在地'].to_s.include?(row['place'])
        end
        if yochien_shiritsuyochien_row
          column_values[0].value = yochien_shiritsuyochien_row['施設名称']
          column_values[2].value = yochien_shiritsuyochien_row['郵便番号']
          column_values[3].value = yochien_shiritsuyochien_row['区']
          column_values[4].value = yochien_shiritsuyochien_row['所在地']
          column_values[5].value = yochien_shiritsuyochien_row['緯度']
          column_values[6].value = yochien_shiritsuyochien_row['経度']
          column_values[7].value = yochien_shiritsuyochien_row['電話番号']
        end
    ...
    def destroy_facility_yochien_shiritsuyochien_docs
      Pippi::Joruri::Relation::Facility::YochienShiritsuyochien.each_with_index do |item, idx|
