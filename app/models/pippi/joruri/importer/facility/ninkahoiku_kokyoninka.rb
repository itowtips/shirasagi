module Pippi::Joruri::Importer::Facility
  class NinkahoikuKokyoninka < Pippi::Joruri::Importer::Base
    ...
    def import_facility_ninkahoiku_kokyoninka_docs
      csv = CSV.open("import_facility_ninkahoiku_kokyoninka_#{Time.zone.now.to_i}.csv",'w')
      path = ::File.join(csv_path, "facility/facilities.csv")
      facility_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      path = ::File.join(csv_path, "facility/ninkahoiku_kokyoninka.csv")
      ninkahoiku_kokyoninka_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      facility_csv.each_with_index do |row, idx|
        next unless row['category_id'] == '43'
        original_id = row["id"]
        ...
        node = ninkahoiku_nodes['公立認可保育園一覧']
        # category = ninkahoiku_nodes[category]
        category = ninkahoiku_nodes['公立認可保育園一覧']
        group = groups[group]
        user = groups[user]
        ...
        rel_Joruri = Pippi::Joruri::Relation::Facility::NinkahoikuKokyoninka.where(joruri_id: original_id).first
        if rel_Joruri
          item = rel_Joruri.owner_item
          next
        else
          item = Article::Page.new
          rel_Joruri = Pippi::Joruri::Relation::Facility::NinkahoikuKokyoninka.new
        end
        ...
        ninkahoiku_kokyoninka_row = ninkahoiku_kokyoninka_csv.find do |ninkahoiku_kokyoninka_row|
          next true if ninkahoiku_kokyoninka_row['施設名称'] == title
          ninkahoiku_kokyoninka_row['所在地'].to_s.include?(row['place'])
        end
        column_values[0].value = ninkahoiku_kokyoninka_row['施設名称']
        column_values[2].value = ninkahoiku_kokyoninka_row['郵便番号']
        column_values[3].value = ninkahoiku_kokyoninka_row['区']
        column_values[4].value = ninkahoiku_kokyoninka_row['所在地']
        column_values[5].value = ninkahoiku_kokyoninka_row['緯度']
        column_values[6].value = ninkahoiku_kokyoninka_row['経度']
        column_values[7].value = ninkahoiku_kokyoninka_row['電話番号']
    ...
    def destroy_facility_ninkahoiku_kokyoninka_docs
      Pippi::Joruri::Relation::Facility::NinkahoikuKokyoninka.each_with_index do |item, idx|
