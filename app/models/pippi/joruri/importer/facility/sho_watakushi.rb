module Pippi::Joruri::Importer::Facility
  class ShoWatakushi < Pippi::Joruri::Importer::Base
    ...
    def import_facility_sho_watakushi_docs
      csv = CSV.open("import_facility_sho_watakushi_#{Time.zone.now.to_i}.csv",'w')
      path = ::File.join(csv_path, "facility/facilities.csv")
      facility_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      path = ::File.join(csv_path, "facility/sho_watakushi.csv")
      sho_watakushi_csv = ::CSV.read(path, headers: true, encoding: 'BOM|UTF-8')
      facility_csv.each_with_index do |row, idx|
        next unless row['category_id'] == '83'
        original_id = row["id"]
        ...
        node = sho_nodes['私立小学校']
        # category = sho_categories[category]
        category = sho_nodes['私立小学校']
        group = groups[group]
        user = groups[user]
        ...
        rel_Joruri = Pippi::Joruri::Relation::Facility::ShoWatakushi.where(joruri_id: original_id).first
        if rel_Joruri
          item = rel_Joruri.owner_item
          next
        else
          item = Article::Page.new
          rel_Joruri = Pippi::Joruri::Relation::Facility::ShoWatakushi.new
        end
        ...
        sho_watakushi_row = sho_watakushi_csv.find do |sho_watakushi_row|
          next true if sho_watakushi_row['名称'] == title
          sho_watakushi_row['所在地'].to_s.end_with?(row['place'])
        end
        column_values[0].value = sho_watakushi_row['名称']
        column_values[1].value = sho_watakushi_row['カナ']
        column_values[2].value = sho_watakushi_row['郵便番号']
        column_values[3].value = sho_watakushi_row['区']
        column_values[4].value = sho_watakushi_row['所在地']
        column_values[5].value = sho_watakushi_row['緯度']
        column_values[6].value = sho_watakushi_row['経度']
        column_values[7].value = sho_watakushi_row['電話番号']
        column_values[8].link_url = sho_watakushi_row['URL']
        column_values[9].value = sho_watakushi_row['設置主体']
        column_values[10].value = sho_watakushi_row['文科省学校コード']
        column_values[11].value = sho_watakushi_row['町名1']
        column_values[12].value = sho_watakushi_row['区名2']
        column_values[13].value = sho_watakushi_row['町名2']
        column_values[14].value = sho_watakushi_row['指定中学校']
        column_values[15].values = sho_watakushi_row['障害種別'].to_s.split('・')
        column_values[16].values = sho_watakushi_row['対象'].to_s.split('・')
    ...
    def destroy_facility_sho_watakushi_docs
      Pippi::Joruri::Relation::Facility::ShoWatakushi.each_with_index do |item, idx|
