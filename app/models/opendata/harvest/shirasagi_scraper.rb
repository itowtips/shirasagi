class Opendata::Harvest::ShirasagiScraper
  ...
  def get_dataset(dataset_url)
    ...
    datetime_text = datetime_text.scan(/(\d+?)年(\d+?)月(\d+?)日 (\d+?)時(\d+?)分/).flatten
    datetime = Time.zone.parse("#{datetime_text[0]}/#{datetime_text[1]}/#{datetime_text[2]} #{datetime_text[3]}:#{datetime_text[4]}")
    dataset["updated"] = datetime
