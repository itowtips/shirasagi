      ...
      loading: function (isLoading, view) {
        if (isLoading) {
          if ($(selector).hasClass("fc-list-format")) {
            var $span = $('<span />', { class: "fc-loading" }).html(i18next.t("gws/schedule.loading"));
            return $(this).find('.fc-view').prepend($span);
          } else {
            var $span = $('<span />', { class: "fc-loading" }).html(i18next.t("gws/schedule.loading"));
            return $(this).find('.fc-widget-content').eq(0).prepend($span);
          }
        } else {
  ...
  Gws_Schedule_Calendar.tapMenuParams = function (selector, opts) {
    ...
    return {
      dayClick: function (date, event, view) {
        ...
        if (opts['tapMenu']) {
          var now = new Date;
          var start = (date.format()) + "T" + (now.getHours()) + ":00:00";
          var state = ("calendar[date]=" + (date.format()) + "&") + Gws_Schedule_Calendar.viewStateQuery(view);
          var planUrl = url + "/new?start=" + start + "&" + state;
          var $addPlan = $("<a />", { href: planUrl, class: 'add-plan' }).html(i18next.t('gws/schedule.links.add_plan'));
          links = $addPlan.prop("outerHTML");

          var todo = url.replace(/schedule\/.*/, 'schedule/todo/-/readables');
          todo += "/new?start=" + start + "&" + state
        ...
        if ($('#calendar-controller').length === 0) {
          ...
          if (view.name !== 'basicWeek') {
            links += $('<a href="" data-view="basicWeek"/>').html(i18next.t("gws/schedule.links.show_week")).prop("outerHTML");
          }
          if (view.name !== 'agendaDay') {
            links += $('<a href="" data-view="agendaDay"/>').html(i18next.t("gws/schedule.links.show_day")).prop("outerHTML");
          }
        }
        if (links) {
  ...
  Gws_Schedule_Calendar.editableParams = function (selector, opts) {
    ...
    return {
      ...
      eventClick: function (event, jsEvent, view) {
        ...
        var target = $(this);
        var $popupContent = $("<div />", { class: 'fc-popup' }).append($("<span />", { class: 'fc-loading' }).html(i18next.t('gws/schedule.loading')));
