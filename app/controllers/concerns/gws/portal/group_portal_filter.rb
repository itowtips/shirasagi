module Gws::Portal::GroupPortalFilter
  ...
  # 必須設定のポートレットは削除できない
  def destroy_all
    @selected_items = @items = @items.to_a.select { |item| !item.required_by_preset? }
  ...
  def set_portal_setting
    ...
    @portal_group = Gws::Group.find(params[:group])
    @portal = @portal_group.find_portal_setting(cur_user: @cur_user, cur_site: @cur_site, cur_group: @cur_group)
