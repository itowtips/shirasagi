module JobDb::BaseFilter
  ...
  included do
    ...
    before_action :set_job_db_assets
    before_action :set_current_site
    # before_action :set_current_group
    before_action :set_account_menu
    before_action :set_crumbs
    navi_view "job_db/main/navi"
  end

  private
    def set_job_db_assets
      SS.config.job_db.stylesheets.each { |m| stylesheet(m) }
      SS.config.job_db.javascripts.each { |m| javascript(m) }
    end

    def set_current_site
      @ss_mode = :job_db
      # @cur_site = JobDb::Site.find params[:site]
      # @cur_user.cur_site = @cur_site
      # @crumbs << [ @cur_site.name, job_db_portal_path ]
    end
