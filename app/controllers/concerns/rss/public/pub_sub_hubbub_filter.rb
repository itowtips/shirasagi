module Rss::Public::PubSubHubbubFilter
  ...
  public
    def pages
      @model.site(@cur_site).public(@cur_date).where(@cur_node.condition_hash)
    end
    ...
    def subscription
      job_model.call_async(@cur_site.host, @cur_node.id, @cur_user.try(:id), @item.id) do |job|
        job.site_id = @cur_site.id
        job.user_id = @cur_user.id if @cur_user
      end
