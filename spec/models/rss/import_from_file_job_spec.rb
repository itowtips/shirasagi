...
describe Rss::ImportFromFileJob, dbscope: :example do
  context "when importing weather sample xml" do
    ...
    it do
      expect { described_class.new.call(site.host, node.id, nil, file.id) }.to change { Rss::Page.count }.from(0).to(2)
      item = Rss::Page.where(rss_link: 'http://*/*/8e55b8d8-518b-3dc9-9156-7e87c001d7b5.xml').first
