...
describe "article_pages", type: :feature, dbscope: :example, js: true, fragile: true do
  ...
  context "opendata_ref/reouset_set_to_manual" do
    ...
    it do
      ...
      #
      # Before closing an article page, dataset and its resource set to none-auto.
      #
      Opendata::Dataset.site(od_site).first.tap do |dataset|
        dataset.assoc_method = 'none'
        dataset.save!
        dataset.resources.each do |resource|
          resource.assoc_method = 'none'
          resource.save!
        end
      end
      ...
      #
      # dataset is still public and its resources are also held.
      #
      expect(Opendata::Dataset.site(od_site).count).to eq 1
      Opendata::Dataset.site(od_site).first.tap do |dataset|
        expect(dataset.name).to eq article_page.name
