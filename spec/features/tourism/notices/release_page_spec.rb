...
describe "my_group", type: :feature, dbscope: :example, js: true do
  ...
  context "with toursim/page" do
    let(:release_date) { Time.zone.at(1.day.from_now.to_i) }
    let!(:layout) { create_cms_layout }

    let!(:node) { create_once :tourism_node_notice, filename: "docs", name: "tourism" }
    let!(:item) { create(:tourism_notice, cur_node: node, layout_id: layout.id, facility: facility_page, state: 'closed', release_date: release_date) }
    ...
    let(:show_path) { tourism_notice_path(site, node, item) }
