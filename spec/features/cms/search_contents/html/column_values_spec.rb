...
describe "cms_search", type: :feature, dbscope: :example, js: true do
  ...
  context "with auth" do
    ...
    context "search column_values" do
      ...
      let!(:page4) do
        create(
          ...
          column_values: [ column4.value_type.new(column: column4, link_label: "リンクラベル", link_url: "https://shirasagi.example.jp") ]
        )
      end
      let!(:page5) do
        create(
      ...
      it "replace cms_column_url_field2 (link_url) with string" do
        ...
        expect(current_path).not_to eq sns_login_path
        within "form.index-search" do
          fill_in "keyword", with: "https://shirasagi.example.jp"
          click_button I18n.t('ss.buttons.search')
        end
        wait_for_ajax
        expect(page).to have_no_css(".result table a", text: "[TEST]page1")
        ...
        page.accept_confirm do
          within "form.index-search" do
            fill_in "keyword", with: "https://shirasagi.example.jp"
            fill_in "replacement", with: "https://aosagi.example.jp"
            click_button I18n.t("ss.buttons.replace_all")
          end
        end
        expect(page).to have_css('#notice', text: I18n.t('ss.notice.saved'))
        ...
        within "form.index-search" do
          fill_in "keyword", with: "https://aosagi.example.jp"
          click_button I18n.t('ss.buttons.search')
        end
        wait_for_ajax
        expect(page).to have_no_css(".result table a", text: "[TEST]page1")
        ...
        page4.reload
        expect(page4.column_values[0].link_url).to eq "https://aosagi.example.jp"
      end
