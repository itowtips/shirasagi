...
describe 'members/agents/nodes/my_anpi_post', type: :feature, dbscope: :example do
  ...
  let(:node_my_anpi_post) { create :member_node_my_anpi_post, cur_site: cms_site, cur_node: node_mypage }
  let(:node_login) { create :member_node_login, site: cms_site, form_auth: 'enabled', redirect_url: node_my_anpi_post.url }
  let(:index_url) { node_my_anpi_post.url }
  let(:text) { unique_id }

  around do |example|
    save = Capybara.app_host
    Capybara.app_host = "http://#{site.domain}"

    example.run

    Capybara.app_host = save
  end
