...
describe "gws/workflow/files", type: :request, dbscope: :example do
  ...
  shared_examples "what riken usecase on gws/workflow/files is" do
      ...
      it do
        ...
        JSON.parse(response.body).tap do |json|
          ...
          json[0].tap do |item_json|
            ...
            expect(item_json["workflow_user"]).to \
              include(
                "name_en" => user1.i18n_name_translations[:en],
                "name_ja" => user1.i18n_name_translations[:ja],
                "uid" => user1.uid,
                "email" => user1.email
              )
            ...
            item_json["workflow_approvers"].first.tap do |approver_json|
              ...
              expect(approver_json["user"]).to \
                include(
                  "name_en" => user2.i18n_name_translations[:en],
                  "name_ja" => user2.i18n_name_translations[:ja],
                  "uid" => user2.uid,
                  "email" => user2.email
                )
            end
  ...
  context "with jwt bearer grant type with service application" do
    ...
    before do
      ...
      application = SS::OAuth2::Application::Service.create!(
        ...
        public_key_type: "rsa", public_key_encrypted: SS::Crypt.encrypt(key.public_key.to_pem)
      )
    ...
    include_context "what riken usecase on gws/workflow/files is"
  end
end
