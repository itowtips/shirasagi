...
describe 'file_repairer:check_states', dbscope: :example do
  ...
  context "no errors" do
    ...
    it do
      ...
      expect(table.headers).to eq csv_header1
      ...
      table = CSV.read(repairer.csv_path, headers: true, encoding: 'BOM|UTF-8')
      expect(table.headers).to eq csv_header2
      expect(table.size).to eq 0
  ...
  context "page and file state unmatched" do
    ...
    it do
      ...
      files = {}
      table.each do |row|
        files[row["ファイルID"].to_i] = row["修復"]
      end
      expect(files[ss_file1.id]).to eq "ファイルの state, site を修復"
      expect(files[ss_file2.id]).to eq "ファイルの state, site を修復"
      expect(files[ss_file3.id]).to eq "ファイルの state, site を修復"
      expect(files[ss_file4.id]).to eq "ファイルの state, site を修復"
      expect(files[ss_file5.id]).to eq "ファイルの state, site を修復"
      expect(files[ss_file6.id]).to eq "ファイルの state, site を修復"
  ...
  context "file_id unincluded in file_ids" do
    ...
    it do
      ...
      expect(files[ss_file5.id]).to eq "file_ids から外れている"
      expect(files[ss_file6.id]).to eq "file_ids から外れている"
      ...
      expect(files[ss_file3.id]).to eq nil
      expect(files[ss_file4.id]).to eq nil
      expect(files[ss_file5.id]).to eq "ファイルIDを file_ids に含めて修復"
      expect(files[ss_file6.id]).to eq "ファイルIDを file_ids に含めて修復"
  ...
  context "private file not found or empty (not fix)" do
    ...
    it do
      ...
      expect(files[ss_file1.id]).to eq "ファイル自体が存在しない"
      expect(files[ss_file2.id]).to eq "privateファイルが空"
      expect(files[ss_file3.id]).to eq "privateファイルが空"
      expect(files[ss_file4.id]).to eq nil
      expect(files[ss_file5.id]).to eq nil
      expect(files[ss_file6.id]).to eq nil
